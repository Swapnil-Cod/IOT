...
void mqttCallback(char* topic, byte* payload, unsigned int len) {
  ...
  if (String(topic) == TOPIC_SUB) {
    if (msg == "on") {
      ...
      // 1) Erase stored Wi-Fi credentials
      prefs.begin("wifi", false);
      prefs.clear();  // clears entire "wifi" namespace
      prefs.end();
      // 2) Acknowledge
      mqtt.publish(TOPIC_SUB, "resetting");
      // 3) Restart so it comes up in AP-provisioning mode
      delay(100);
      ESP.restart();
    }
  }
...
void setup() {
  ...
  // attach Wi-Fi event handlers
  WiFi.onEvent(onWiFiGotIP,        ARDUINO_EVENT_WIFI_STA_GOT_IP);
  WiFi.onEvent(onWiFiDisconnected, ARDUINO_EVENT_WIFI_STA_DISCONNECTED);

  // provisioning or auto-join
  if (loadCredentials()) {
...
void loop() {
  ...
  // blink LED until both Wi-Fi & MQTT are up
  if (!(wifiOK && mqttOK)) {
